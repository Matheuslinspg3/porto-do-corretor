...
-- A4: Atomic accept-invite RPC
-- Replaces sequential edge function operations with a single transactional function
...
BEGIN
  ...
  IF v_current_org_id IS NOT NULL AND v_current_org_id != v_invite.organization_id THEN
    ...
    IF v_member_count = 1 THEN
      -- Only member: safe to archive (mark inactive instead of deleting)
  ...
  -- 5. Replace roles atomically
  DELETE FROM user_roles WHERE user_id = p_user_id;
  INSERT INTO user_roles (user_id, role, organization_id)
  VALUES (p_user_id, v_invite.role, v_invite.organization_id);
